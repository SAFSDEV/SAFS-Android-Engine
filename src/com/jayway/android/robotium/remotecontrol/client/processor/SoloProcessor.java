 ...
 * 		   May 17, 2013		(Lei Wang)	Update to support Robotium 4.1<br>
 * 		   May 23, 2013		(Lei Wang)	Modify method takeScreenshot()<br>
 * 		   Jun 21, 2013		(Lei Wang)	Update to support Robotium 4.1+<br>
 * 		   Jun 25, 2013		(Carl Nagle)	Update to support Robotium 4.2<br>
 * 		   Jul 18, 2013		(Lei Wang)	Modify method getViewByName(): try to get view under package 'android'.<br>
 * 		   SEP 04, 2013		(Carl Nagle)	Update to support getViewId(int, index)<br>
 * 		   MAR 13, 2014		(Carl Nagle)	Update to support Robotium 5.x<br>
 * 		   JUL 21, 2014		(Lei Wang)	Update to support Robotium 5.2.1<br>
 * 		   MAR 05, 2015		(Lei Wang)	Update to support Robotium 5.3.1<br>
 */
public class SoloProcessor extends AbstractProcessor implements CacheReferenceInterface{
	...
	/**
	 * The instance of {@link com.robotium.solo.RCSolo}, it is used<br>
	...
	/**
	 * local cache for containing the {@link View#getTag()}<br>
	 * <b>Note:</b> Don't manipulate it directly like tagCache.get(key) etc.<br>
	 * Use the cache-manipulation-methods defined in {@link AbstractProcessor}<br>
	 * @see AbstractProcessor#getCachedItem(Hashtable, Object)
	 * @see AbstractProcessor#removeCachedItem(Hashtable, Object)
	 * @see AbstractProcessor#putCachedItem(Hashtable, Object, Object)
	 */
	protected Hashtable<String,Object> tagCache = new Hashtable<String,Object>(INITIAL_CACHE_SIZE);
	...
	public void processProperties(Properties props){
		...
		try{
			if(remoteCommand.equals(SoloMessage.cmd_startmainlauncher)){
				...
				if(remoteCommand.equals(SoloMessage.cmd_assertcurrentactivityname) ||
				...
				}else if(remoteCommand.equals(SoloMessage.cmd_getviewbytag) ||
						 remoteCommand.equals(SoloMessage.cmd_getviewbytagmatch)){
					getViewByTag(props);
				...
				}else if(remoteCommand.equals(SoloMessage.cmd_presssoftkeyboardnextbutton)){
					pressSoftKeyboardNextButton(props);
					
				}else if(remoteCommand.equals(SoloMessage.cmd_presssoftkeyboardsearchbutton)){
					pressSoftKeyboardSearchButton(props);
				...
				}else if(remoteCommand.equals(SoloMessage.cmd_setmobiledata)){
					setMobileData(props);
					
				}else if(remoteCommand.equals(SoloMessage.cmd_setwifidata)){
					setWiFiData(props);
				...
				}else if(remoteCommand.equals(SoloMessage.cmd_waitforemptyactivitystack)){
					waitForEmptyActivityStack(props);
				...
				}else if(remoteCommand.equals(SoloMessage.cmd_waitforviewbytagreference) ||
						remoteCommand.equals(SoloMessage.cmd_waitforviewbytagreferencetimeout) ||
						remoteCommand.equals(SoloMessage.cmd_waitforviewbytagreferencetimeoutscroll)){
					waitForViewByTag(props);
					...
					typeTextInWebElementByUID(props);
						...
						remoteCommand.equals(SoloMessage.cmd_getwebelements) ||
						remoteCommand.equals(SoloMessage.cmd_getcurrentwebelementsby) ||
						remoteCommand.equals(SoloMessage.cmd_getwebelementsby) ||
				...
				}else if(remoteCommand.equals(SoloMessage.cmd_gettagforview) ){
					getTagForView(props);
					...
					handleZoomRotateSwipe(props);
						 ...
						 remoteCommand.equals(SoloMessage.cmd_clickonactionbarhomebutton) ||
						 remoteCommand.equals(SoloMessage.cmd_unlockscreen)){
					handleComandWithoutParams(props);
					
				}else if(remoteCommand.equals(SoloMessage.cmd_setnavigationdrawer)){
					setNavigationDrawer(props);					
					
				}else if(remoteCommand.equals(SoloMessage.cmd_config_gettimeoutsmall) ||
						 remoteCommand.equals(SoloMessage.cmd_config_gettimeoutlarge) ||
						 remoteCommand.equals(SoloMessage.cmd_config_settimeoutsmall) ||
						 remoteCommand.equals(SoloMessage.cmd_config_settimeoutlarge)){
					handleConfigTimeoutCommands(props);
					
				}else if(remoteCommand.equals(SoloMessage.cmd_config_getshouldscroll) ||
						 remoteCommand.equals(SoloMessage.cmd_config_setshouldscroll)){
					handleConfigShouldScrollCommands(props);
					
				}else if(remoteCommand.equals(SoloMessage.cmd_config_getusejavascripttoclickwebelements) ||
						 remoteCommand.equals(SoloMessage.cmd_config_setusejavascripttoclickwebelements)){
					handleConfigUseJavaScriptToClickWebElementsCommands(props);
					
				}else if(remoteCommand.equals(SoloMessage.cmd_config_getscreenshotsavepath) ||
						 remoteCommand.equals(SoloMessage.cmd_config_setscreenshotsavepath)){
					handleConfigScreenshotSavePathCommands(props);
					
				}else if(remoteCommand.equals(SoloMessage.cmd_config_getscreenshotfiletype) ||
						 remoteCommand.equals(SoloMessage.cmd_config_setscreenshotfiletype)){
					handleConfigScreenshotFileTypeCommands(props);
					
				}else{
					debug(debugPrefix +"Unknown command '"+remoteCommand+"'.");
					// "unknown/not executed" result already set.
				}
			}
			...
			debug(remoteCommand+": Met Throwable: '"+e.getClass().getSimpleName()+":"+e.getMessage()+"'. This maybe a program error!!!");
			setGeneralError(props, SoloMessage.RESULT_INFO_EXCEPTION, e.getClass().getSimpleName()+":"+e.getMessage());
		}
	...
	private void setSuccessResultForView(Properties props, View view) throws ProcessorException{
		...
		if(view==null) throw new ProcessorException(SoloMessage.RESULT_INFO_VIEW_NULL);			
...
//		}
	 ...
	 * {@link Solo#clickOnActionBarHomeButton()} Robotium 3.4.1<br>
	 * {@link Solo#unlockScreen()} Robotium 5.2.1<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters
	 */
	void handleComandWithoutParams(Properties props){
		...
		try{
			...
			}else if(remoteCommand.equals(SoloMessage.cmd_unlockscreen)){
				solo.unlockScreen();
			...
			debug("handleComandWithoutParams(): '"+remoteCommand+"'"+SoloMessage.getStackTrace(x));
	 ...
	 * {@link Solo#getCurrentViews(Class, boolean)} - Robotium 5.2.1<br>
	 * {@link Solo#getCurrentViews(Class, boolean, View)} - Robotium 5.2.1<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters
	 */
	void getCurrentViews(Properties props) throws ProcessorException{
		...
		if(remoteCommand.equals(SoloMessage.cmd_getcurrentbuttons)){
			...
			try{ currentViewList = solo.getCurrentViews(NumberPicker.class); }
			catch(Throwable apiTooLow){
				throw new ProcessorException(remoteCommand+" cannot seek NumberPickers at this Android API level.");			
			}
			...
			if(SoloMessage.exist(props, SoloMessage.PARAM_INCLUDSUBCLASSES)){
				boolean includeSubclasses = SoloMessage.getBoolean(props, SoloMessage.PARAM_INCLUDSUBCLASSES);
				currentViewList = solo.getCurrentViews(viewClass, includeSubclasses);
			...
			if(SoloMessage.exist(props, SoloMessage.PARAM_INCLUDSUBCLASSES)){
				boolean includeSubclasses = SoloMessage.getBoolean(props, SoloMessage.PARAM_INCLUDSUBCLASSES);
				currentViewList = solo.getCurrentViews(viewClass, includeSubclasses, parent);
	 ...
	 * {@link Solo#getWebElements()} -- Robotium 5.1<br>
	 * {@link Solo#getWebElements(By)} -- Robotium 5.1<br>
	...
	void getCurrentWebElements(Properties props) throws ProcessorException{
		...
		}else if(remoteCommand.equals(SoloMessage.cmd_getwebelements)){
			list = solo.getWebElements();
			
		}else if(remoteCommand.equals(SoloMessage.cmd_getwebelementsby)){
	 ...
	 * The optional index is given by parameter<br>
	 * Solo will get the appropriate View.<br>
	 * 
	 * <p>
	 * calling:<br>
	 * {@link #getViewById(int, Class)}<br>
	 * {@link #getViewById(int, int, Class)}<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters
	 */
	void getViewById(Properties props) throws ProcessorException{
		...
		int index = -1;
		try{ index = SoloMessage.getInteger(props, SoloMessage.PARAM_INDEX);}
		catch(ProcessorException optional){ /*ignore optional parameter exception */ }

		View view = null;
		if(index < 0){
			debug(debugPrefix+" Try to get view for id '"+id+"'");
			//getViewById() will never return a null
			view = getViewById(id, null);
		}else{
			debug(debugPrefix+" Try to get view id '"+id+"' at index '"+ index +"'");
			//getViewById() will never return a null
			view = getViewById(id, index, null);
		}
	...
	/**
	 * get the {@link android.view.View} by Tag Object. <b>Requires Robotium 5.3.0+</b><br>
	 * <p>
	 * calling:<br>
	 * {@link Solo#getView(Object)}<br>
	 * {@link Solo#getView(Object, int)}<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters
	 */
	void getViewByTag(Properties props) throws ProcessorException{
		String debugmsg = TAG+".getViewByTag(Properties) ";
		String taguid = SoloMessage.getString(props, SoloMessage.PARAM_REFERENCE);

		Object tag = getTagByUID(taguid);
		debug(debugmsg+"Got Tag Object ="+tag);
		
		View view = null;
		if(tag!=null){
			if(remoteCommand.equals(SoloMessage.cmd_getviewbytag)){
				view = solo.getView(tag);
			}else if(remoteCommand.equals(SoloMessage.cmd_getviewbytagmatch)){
				int index = SoloMessage.getInteger(props, SoloMessage.PARAM_INDEX);
				view = solo.getView(tag, index);
			}else{
				throw new ProcessorException(remoteCommand+" could not be processed in getViewByTag().");				
			}
		}else{
			throw new ProcessorException(SoloMessage.RESULT_INFO_VIEW_TAG_INVALID);
		}
	 ...
	 * current package, or the android package.<br>
	 * given by {@link SoloMessage#PARAM_RESID} or {@link SoloMessage#PARAM_ID}<br>
	 * 
	 * <p>
	 * calling:<br>
	 * {@link Solo#getString(int)}<br>
	 * {@link Solo#getString(String)}<br>
	 */
	void getString(Properties props) throws ProcessorException{
		...
		String strId = null;
		try{ 
			resId = SoloMessage.getInteger(props, SoloMessage.PARAM_RESID); 
			debug("Params int resId="+resId);
		}catch(ProcessorException option){
			// will throw ProcessorException if not present
			strId = SoloMessage.getString(props, SoloMessage.PARAM_ID); 
			debug("Params String Id="+strId);
		}
		
		String resourceStr = "";
		String msgStr = (strId == null) ? String.valueOf(resId): strId;
		try{			
			if( strId == null) resourceStr = solo.getString(resId);
			else resourceStr = solo.getString(strId); // Robotium 4.3 or later
		}catch(NotFoundException nfe){
			debug(debugPrefix +" Can't find resource string for id '"+msgStr+"'. Exception: "+nfe.getMessage());
			resourceStr = "null";
		}catch(Exception noSuchMethod){
			debug(debugPrefix +" Upgrade to Robotium version 4.3 or later. Exception: "+noSuchMethod.getMessage());
			resourceStr = "null";
		}
	...
	/**
	 * Press the soft keyboard Next button.<br>
	 * Requires Robotium 5.0.1
	 * <p>
	 * calling:<br>
	 * {@link Solo#pressSoftKeyboardNextButton()}<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters
	 */
	void pressSoftKeyboardNextButton(Properties props){
		solo.pressSoftKeyboardNextButton();
		setGeneralSuccess(props);
	}
	
	/**
	 * Press the soft keyboard Search button.<br>
	 * Requires Robotium 5.x
	 * <p>
	 * calling:<br>
	 * {@link Solo#pressSoftKeyboardSearchButton()}<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters
	 */
	void pressSoftKeyboardSearchButton(Properties props){
	 ...
	 * 
	 * scrollPosition is an optional parameter.<br>
	 * 
	 * <p>
	 * calling:<br>
	 * {@link Solo#scrollToSide(int)}<br>
	 * {@link Solo#scrollViewToSide(View, int)} -- Robotium 3.6<br>
	 * {@link Solo#scrollToSide(int, float)} -- Robotium 4.3<br>
	 * {@link Solo#scrollToSide(int, float, int)} -- Robotium 5.2.1<br>
	 * {@link Solo#scrollViewToSide(View, int, float)} -- Robotium 4.3<br>
	 * {@link Solo#scrollViewToSide(View, int, float, int)} -- Robotium 5.2.1<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters
	 */
	void scrollToSide(Properties props) throws ProcessorException{
		...
		float scrollPosition = Float.NaN;
		try{
			scrollPosition = SoloMessage.getFloat(props, SoloMessage.PARAM_SCROLLPOSITION);
			debug("Params: scrollPosition="+scrollPosition);
		}catch(Exception optionalParam){scrollPosition = Float.NaN;}
		
		int stepCount = Integer.MIN_VALUE;//a valid step count is never negative
		try{
			stepCount = SoloMessage.getInteger(props, SoloMessage.PARAM_STEPCOUNT);
			debug("Params: stepCount="+stepCount);
		}catch(Exception optionalParam){stepCount = Integer.MIN_VALUE; }
		
		if(remoteCommand.equals(SoloMessage.cmd_scrolltoside)){
			if(scrollPosition==Float.NaN) solo.scrollToSide(side);
			else{
				if(stepCount==Integer.MIN_VALUE){
					solo.scrollToSide(side, scrollPosition);
				}else{
					solo.scrollToSide(side, scrollPosition, stepCount);
				}
			}
			...
			if(scrollPosition==Float.NaN) solo.scrollViewToSide(view, side);
			else{
				if(stepCount==Integer.MIN_VALUE){
					solo.scrollViewToSide(view, side, scrollPosition);
				}else{
					solo.scrollViewToSide(view, side, scrollPosition, stepCount);
				}
			}
		}
		
		if(com.robotium.solo.Solo.RIGHT==side){
	...
	/**
	 * Turn on/off 'mobile data'<br>
	 * 
	 * <p>
	 * calling:<br>
	 * {@link Solo#setMobileData(Boolean)} -- Robotium 5.3.1<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters
	 */
	void setMobileData(Properties props) throws ProcessorException{
		boolean turnedOn = SoloMessage.getBoolean(props, SoloMessage.PARAM_TURNON);
		String resultInfo = "turnedOn="+turnedOn;
	...
	/**
	 * Turn on/off 'wifi data'<br>
	 * 
	 * <p>
	 * calling:<br>
	 * {@link Solo#setWiFiData(Boolean)}  -- Robotium 5.3.1<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters
	 */
	void setWiFiData(Properties props) throws ProcessorException{
		boolean turnedOn = SoloMessage.getBoolean(props, SoloMessage.PARAM_TURNON);
		String resultInfo = "turnedOn="+turnedOn;
	...
	/**
	 * Sets the status of the {@link NavigationDrawer}. 
	 * Examples of status are: {@code Solo.CLOSED} and {@code Solo.OPENED}.
	 *
	 * <p>
	 * calling:<br>
	 * {@link Solo#setNavigationDrawer(int)}<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters
	 */
	void setNavigationDrawer(Properties props) throws ProcessorException{
		int status = SoloMessage.getInteger(props, SoloMessage.PARAM_STATUS);
		String resultInfo = " value=";
		if(status == solo.OPENED)
			resultInfo += "OPENED";
		else if(status == solo.CLOSED) 
			resultInfo += "CLOSED";
		else{
			throw new ProcessorException(remoteCommand+" could not be processed due to invalid STATUS setting.");
		}
		
		debug("Params: status "+resultInfo);
		solo.setNavigationDrawer(status);
	...
	/**
	 * Wait for Empty Activity Stack.<br>
	 * Requires Robotium 5.0.1
	 * <p>
	 * calling:<br>
	 * {@link Solo#waitForEmptyActivityStack(int)}<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters
	 */
	void waitForEmptyActivityStack(Properties props) throws ProcessorException{
		int timeout = SoloMessage.getInteger(props, SoloMessage.PARAM_TIMEOUT);
		String resultInfo = "";

		debug("Params: time="+timeout);
		
		boolean found = solo.waitForEmptyActivityStack(timeout);
		resultInfo = " in "+timeout+" millis timeout period.";			
		
		if(!found){			
			resultInfo = "Activity Stack NOT empty"+resultInfo;
		}else{
			resultInfo = "Activity Stack empty"+resultInfo;
		}
		debug(resultInfo);		
		setGeneralSuccessWithSpecialInfo(props, String.valueOf(found));
	}
	...
	void waitForDialog(Properties props) throws ProcessorException{
		int timeout = -1;
		try{ timeout = SoloMessage.getInteger(props, SoloMessage.PARAM_TIMEOUT); }
		catch(ProcessorException option){ 
			timeout = Timeout.getLargeTimeout(); 
		}
	 ...
	 * <p>
	 * calling:<br>
	 * {@link Solo#waitForView(Object)} Since Robotium5.3.1 <br>
	 * {@link Solo#waitForView(Object, int, int)} Since Robotium5.3.1 <br>
	 * {@link Solo#waitForView(Object, int, int, boolean)} Since Robotium5.3.1 <br>
	 * 
	 * @param props		The Properties object containing the in and out parameters
	 */
	void waitForViewByTag(Properties props) throws ProcessorException{
		String taguid = SoloMessage.getString(props, SoloMessage.PARAM_REFERENCE);
		debug("Params: tag uid="+taguid);
		...
		if(tag!=null){
			View view = null;
			debug("try to get view by tag object "+tag);
			if(remoteCommand.equals(SoloMessage.cmd_waitforviewbytagreference)){
				synchronized(solo){
					found = solo.waitForView(tag);
					if(found){
						view = solo.getWaitedView();
						resultInfo = view.getClass().getSimpleName();					
					}
				}
			}else if(remoteCommand.equals(SoloMessage.cmd_waitforviewbytagreferencetimeout)){
				int minimumMatches = SoloMessage.getInteger(props, SoloMessage.PARAM_MINIMUMMATCHES);
				int timeout = SoloMessage.getInteger(props, SoloMessage.PARAM_TIMEOUT);
				debug("Params: minimumMatches="+minimumMatches+"; timeout="+timeout);
				synchronized(solo){
					found = solo.waitForView(tag, minimumMatches, timeout);
					if(found){
						view = solo.getWaitedView();
						resultInfo = view.getClass().getSimpleName()+" at least "+minimumMatches+" times in "+timeout+" milliseconds ";
					}
				}

			}else if(remoteCommand.equals(SoloMessage.cmd_waitforviewbytagreferencetimeoutscroll)){
				int minimumMatches = SoloMessage.getInteger(props, SoloMessage.PARAM_MINIMUMMATCHES);
				int timeout = SoloMessage.getInteger(props, SoloMessage.PARAM_TIMEOUT);
				boolean scroll = SoloMessage.getBoolean(props, SoloMessage.PARAM_SCROLL);
				debug("Params: minimumMatches="+minimumMatches+"; timeout="+timeout+"; scroll="+scroll);
				synchronized(solo){
					found = solo.waitForView(tag, minimumMatches, timeout, scroll);
					if(found){
						view = solo.getWaitedView();
						resultInfo = view.getClass().getSimpleName()+" at least "+minimumMatches+" times in "+timeout+" milliseconds ";
					}
			...
			//we will set the PARAM_CLASS of the view for remote caller's reference
			if(view!=null) setViewClassResult(props, view);
		}else{
			debug("couldn't get Tag Object from cache.");
		}
	...
	/**
	 * Get the Tag object from the cache. If not found, the taguid itself will be considered as a String Tag object.
	 */
	private Object getTagByUID(String taguid){
		Object tag = null;
		if(taguid==null || taguid.isEmpty()) return null;
		tag = getCachedItem(tagCache, taguid);
		if(tag==null) tag = getCachedObject(taguid, true);//should we get it from all caches?
		if(tag==null) tag = taguid;//taguid will be considered as a string Tag object.
		return tag;
	}
	 ...
	 * The real Tag object will be stored in cache {@link #tagCache}.
	 * @param props		The Properties object containing the in and out parameters
	 */
	void getTagForView(Properties props) throws ProcessorException{
		String uid = SoloMessage.getString(props, SoloMessage.PARAM_REFERENCE);
		debug("Params: uid="+uid);
		...
		if(remoteCommand.equals(SoloMessage.cmd_gettagforview)){
			Object tag = view.getTag();
			debug("found view's tag = "+tag);

			if(tag==null){
				debug(" Can NOT get Tag object for view "+ view.getClass().getSimpleName());
				throw new ProcessorException(SoloMessage.RESULT_INFO_VIEW_TAG_INVALID);	
			}

			//Get the unique key for Tag object from cache
			String taguid = convertToKey(tagCache, tag);
			if(taguid==null){
				debug(" Can NOT generate UID for Tag of view "+ view.getClass().getSimpleName());
				throw new ProcessorException(SoloMessage.RESULT_INFO_GENERATE_UID_NULL);	
			}else{
				debug(" get uid '"+uid+"' for Tag of view "+view.getClass().getSimpleName());
			}

			setGeneralSuccessWithSpecialInfo(props, taguid);
		}else{
			throw new ProcessorException(remoteCommand+" could not be processed in getTagForView().");
		}
	...
	/**
	 * Wait for {@link WebElement}. -- Robotium 4.1<br>
	 * 
	 * <p>
	 * calling:<br>
...
//			WindowManager wm = (WindowManager) inst.getContext().getSystemService(Context.WINDOW_SERVICE);
	 ...
	 * <p>
	 * calling:<br>
	 * {@link Solo#waitForCondition(Condition, int)} -- Robotium 4.1<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters<br>
	...
	void waitForCondition(Properties props) throws ProcessorException{
		try{
			int timeout = SoloMessage.getInteger(props, SoloMessage.PARAM_TIMEOUT);
			String objectStr = SoloMessage.getString(props, SoloMessage.PARAM_OBJECT);
			com.jayway.android.robotium.remotecontrol.Condition condition = (com.jayway.android.robotium.remotecontrol.Condition) SoloMessage.decodeBase64Object(objectStr);
	 ...
	 * <p>
	 * calling:<br>
	 * {@link Solo#clearTextInWebElement(By)} -- Robotium 4.1<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters<br>
	 ...
	 * <p>
	 * calling:<br>
	 * {@link Solo#clickOnWebElement(By)} -- Robotium 4.1<br>
	 * {@link Solo#clickOnWebElement(By, int)} -- Robotium 4.1<br>
	 * {@link Solo#clickOnWebElement(By, int, boolean)} -- Robotium 4.1<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters<br>
	 * 					&lt;in&gt; by the By object. Examples are: {@code By.id("id")} and {@code By.name("name")}.<br>
	 */
	 ...
	 * <p>
	 * calling:<br>
	 * {@link Solo#clickOnWebElement(WebElement)} -- Robotium 4.1<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters<br>
	 ...
	 * <p>
	 * calling:<br>
	 * {@link Solo#enterTextInWebElement(By, String)} -- Robotium 4.1<br>
	 * {@link Solo#typeTextInWebElement(By, String)} -- Robotium 4.1<br>
	 ...
	 * 
	 * @param id				int, This is the view's id according to which we will get a View.
	 * 							This id is the R.id generated when building the AUT.<br>
	 ...
	 * @return 
	 */
	private <T extends View> View getViewById(int id, Class<T> exceptedClass) throws ProcessorException{
		View view = solo.getView(id);
		String exceptedClassName = null;
		
		if(view==null){
			throw new ProcessorException(" View for id '"+id+"' is null.");			
		}
		
		if(!checkClass(view, exceptedClass)){
			throw new ProcessorException(" View for id '"+id+"' is not a '"+exceptedClassName+"'.");
		}
	...
	/**
	 * According to the view's id and index, try to get a View as an 'expected class'.<br>
	 * If not, a {@link ProcessorException} will be thrown out.<br>
	 * If a view is returned, user can CAST it DIRECTLY to 'expected class'<br>
	 * 
	 ...
	 * @param index				int, the nth matching View with the given uid.
	 * @param exceptedClass		Class, The expected class for the View that we will get.
	 *                          If this parameter is null, we won't check the View's class name.
	 * @return 
	 */
	private <T extends View> View getViewById(int uid, int index, Class<T> exceptedClass) throws ProcessorException{
		View view = solo.getView(uid, index);
		String exceptedClassName = null;
		
		if(view==null){
			throw new ProcessorException(" View for id '"+uid+"' at index '"+ index +"' is null.");			
		}
	...
	/**
	 * Handle the GET/SET commands for Solo.Config.timeout_small and Solo.Config.timeout_large. (Robotium 5.0.1)
	 * <p>
	 * calling:<br>
	 * {@link Solo.Config.timeout_small}<br>
	 * {@link Solo.Config.timeout_large}<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters
	 */
	void handleConfigTimeoutCommands(Properties props){

		try{
			int millisecond = -1;

			if(remoteCommand.equals(SoloMessage.cmd_config_settimeoutsmall)){
				millisecond = SoloMessage.getInteger(props, SoloMessage.PARAM_TIMEOUT);
				solo.getConfig().timeout_small= millisecond;
				millisecond = solo.getConfig().timeout_small;				
			}else if (remoteCommand.equals(SoloMessage.cmd_config_settimeoutlarge)) {
				millisecond = SoloMessage.getInteger(props, SoloMessage.PARAM_TIMEOUT);
				solo.getConfig().timeout_large= millisecond;
				millisecond = solo.getConfig().timeout_large;
			}else if (remoteCommand.equals(SoloMessage.cmd_config_gettimeoutsmall)) {
				millisecond = solo.getConfig().timeout_small;				
			}else if (remoteCommand.equals(SoloMessage.cmd_config_gettimeoutlarge)) {
				millisecond = solo.getConfig().timeout_large;
			}			
			setGeneralSuccessWithSpecialInfo(props,String.valueOf(millisecond));
			
		}catch(Throwable x){
			debug("handleConfigTimeouts() "+SoloMessage.getStackTrace(x));
			setGeneralError(props, x.getClass().getSimpleName()+": "+ x.getMessage());
		}
	}
		
	/**
	 * Handle the GET/SET commands for Solo.Config.shouldScroll. (Robotium 5.0.1)
	 * <p>
	 * calling:<br>
	 * {@link Solo.Config.shouldScroll}<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters
	 */
	void handleConfigShouldScrollCommands(Properties props){

		try{
			boolean shouldScroll = false;

			if(remoteCommand.equals(SoloMessage.cmd_config_setshouldscroll)){
				shouldScroll = SoloMessage.getBoolean(props, SoloMessage.PARAM_STATUS);
				solo.getConfig().shouldScroll= shouldScroll;
			}			
			shouldScroll = solo.getConfig().shouldScroll; //GET comand + SET verify			
			setGeneralSuccessWithSpecialInfo(props,String.valueOf(shouldScroll));
			
		}catch(Throwable x){
			debug("handleConfigShouldScrollCommands() "+SoloMessage.getStackTrace(x));
			setGeneralError(props, x.getClass().getSimpleName()+": "+ x.getMessage());
		}
	}
		
	/**
	 * Handle the GET/SET commands for Solo.Config.useJavaScriptToClickWebElements. (Robotium 5.0.1)
	 * <p>
	 * calling:<br>
	 * {@link Solo.Config.useJavaScriptToClickWebElements}<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters
	 */
	void handleConfigUseJavaScriptToClickWebElementsCommands(Properties props){

		try{
			boolean useJavascript = false;

			if(remoteCommand.equals(SoloMessage.cmd_config_setusejavascripttoclickwebelements)){
				useJavascript = SoloMessage.getBoolean(props, SoloMessage.PARAM_STATUS);
				solo.getConfig().useJavaScriptToClickWebElements = useJavascript;
			}			
			useJavascript = solo.getConfig().useJavaScriptToClickWebElements; //GET comand + SET verify			
			setGeneralSuccessWithSpecialInfo(props,String.valueOf(useJavascript));
			
		}catch(Throwable x){
			debug("handleConfigUseJavaScriptToClickWebElementCommands() "+SoloMessage.getStackTrace(x));
			setGeneralError(props, x.getClass().getSimpleName()+": "+ x.getMessage());
		}
	}
		
	/**
	 * Handle the GET/SET commands for Solo.Config.screenshotSavePath. (Robotium 5.0.1)
	 * <p>
	 * calling:<br>
	 * {@link Solo.Config.screenshotSavePath}<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters
	 */
	void handleConfigScreenshotSavePathCommands(Properties props){

		try{
			String savePath = null;
			if(remoteCommand.equals(SoloMessage.cmd_config_setscreenshotsavepath)){
				savePath = SoloMessage.getString(props, SoloMessage.PARAM_TEXT);
				solo.getConfig().screenshotSavePath = savePath;
			}			
			savePath = solo.getConfig().screenshotSavePath; //GET comand + SET verify			
			setGeneralSuccessWithSpecialInfo(props,savePath);
			
		}catch(Throwable x){
			debug("handleConfigScreenshotSavePathCommands() "+SoloMessage.getStackTrace(x));
			setGeneralError(props, x.getClass().getSimpleName()+": "+ x.getMessage());
		}
	}
		
	/**
	 * Handle the GET/SET commands for Solo.Config.screenshotFileType. (Robotium 5.0.1)
	 * <p>
	 * calling:<br>
	 * {@link Solo.Config.screenshotFileType}<br>
	 * 
	 * @param props		The Properties object containing the in and out parameters
	 */
	void handleConfigScreenshotFileTypeCommands(Properties props){

		try{
			String fileType = null;
			if(remoteCommand.equals(SoloMessage.cmd_config_setscreenshotfiletype)){
				fileType = SoloMessage.getString(props, SoloMessage.PARAM_TEXT);
				if(fileType.equalsIgnoreCase(Message.FILETYPE_JPEG))
					solo.getConfig().screenshotFileType = Solo.Config.ScreenshotFileType.JPEG;
				else if(fileType.equalsIgnoreCase(Message.FILETYPE_PNG))
					solo.getConfig().screenshotFileType = Solo.Config.ScreenshotFileType.PNG;
				else{
					throw new ProcessorException("handleConfigScreenShotFileType unrecognized (SET) file type '"+ fileType +"' specified.");					
				}				
			}	
			if(solo.getConfig().screenshotFileType == Solo.Config.ScreenshotFileType.JPEG)
				fileType = Message.FILETYPE_JPEG;
			else if(solo.getConfig().screenshotFileType == Solo.Config.ScreenshotFileType.PNG)
				fileType = Message.FILETYPE_PNG;
			else{
				fileType = String.valueOf(solo.getConfig().screenshotFileType);
			}
			setGeneralSuccessWithSpecialInfo(props,fileType);
			
		}catch(Throwable x){
			debug("handleConfigScreenshotFileTypeCommands() "+SoloMessage.getStackTrace(x));
			setGeneralError(props, x.getClass().getSimpleName()+": "+ x.getMessage());
		}
	}
	...
	public Object getCachedObject(String key, boolean useChain) {
		...
		if(item == null) item = getCachedItem(tagCache, key);
